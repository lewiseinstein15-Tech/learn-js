...
async function fetchtasks (){
    ...
    const raw = await fetch("http://localhost:5500/datatest/");
    ...
    for(day of days){
        ...
        const raw2 = await fetch(`http://localhost:5500/datatest/${day}`);
...
function reload (){
 ...
 function addtask(){
       ...
       try {
      fetch(`http://localhost:5500/addtask/${day}`,{
        method : 'POST',
...
async function marktask(status,index){
       ...
       const day = document.getElementById('ShowSingleDayList').value;
       try {
      fetch(`http://localhost:5500/marktask/${day}`,{
        method : 'POST',
        headers: {'Content-Type':'application/json'},
...
async function deletetask(dayfortask,index){
       ...
       try {
      fetch(`http://localhost:5500/deletetask/${dayfortask}`,{
        method : 'POST',
        headers: {'Content-Type':'application/json'},
